...
class JsonVisualizerFrame(ctk.CTkFrame):
    def __init__(self, parent, classify_screenshot, json_path, save_path, frame_width=600, frame_height=300, font_scale=1, *args, **kwargs):
        ...
        self.configure(width=self.frame_width, height=self.frame_height)
        self.create_widgets()
    ...
    def create_summary_bar(self):
      ...
      self._summary_frame.pack(fill="x", padx=12, pady=12)
      ...
      self.update_totals(on_task, off_task, none)

    def update_totals(self, on_task, off_task, none):
    ...
    def create_squares_grid(self) -> None:
        ...
        self._grid_frame.pack(fill="both", expand=True, padx=12, pady=12)

        # Set up grid
        cols, rows = self.configure_grid_dim(total=len(screenshots))
        for r in range(rows):
        ...
        for idx, shot in enumerate(screenshots):
            ...
            ts = datetime.strptime(fname.replace("screenshot_", "").replace(".png", ""),
                                  "%Y%m%d_%H%M%S")
            time_lbl = ts.strftime("%-I:%M %p (%Y/%m/%d)")
    ...
    def _on_frame_resize(self, event=None):
        ...
        cols, rows = self.configure_grid_dim()
    ...
    def add_screenshot_button(
        ...
        cls         = screenshot_entry["classification"]
        fg, hover   = self.color_map.get(cls, ("dimgray", "gray"))
        print(f"Adding new {cls} screenshot from {time_lbl}")
    ...
    def update_screenshot(self, button, image_path, screenshot_entry, time_label):
        on_task, off_task, none = self.get_totals()
...
if __name__ == "__main__":
    app = ctk.CTk()
    app.title("JSON Visualizer")
    app.geometry("600x300")

    visualizer = JsonVisualizerFrame(app, json_path="testjson.json")
